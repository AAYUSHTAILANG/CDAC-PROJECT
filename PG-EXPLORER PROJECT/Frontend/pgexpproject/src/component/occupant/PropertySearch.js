...
function PropertySearch() {
  ...
  const handleGo = () => {
    ...
    fetch(`http://localhost:8080/searchProperty/${info.area_id}/${occupantId}`)
      .then((response) => {
        if (!response.ok) {
    ...
    fetch(`http://localhost:8080/sendRequest`, {
      ...
      .then((response) => {
        if (!response.ok) {
          ...
          // Remove the requested property from searchResults
          setSearchResults(
            searchResults.filter((property) => property.id !== propertyId)
          );
